...
const SKIP_VALIDATION: Set<string> = new Set(['welcome', 'ending', 'statement', 'schedule'])
...
function runnerReducer(state: RunnerState, action: RunnerAction): RunnerState {
  switch (action.type) {
    ...
    case 'NEXT': {
      ...
      if (!SKIP_VALIDATION.has(currentNode.type)) {
        const result = validateAnswer(currentNode, state.answers[state.currentNodeId] ?? null)
        if (!result.valid) {
          return {
            ...state,
            errors: { ...state.errors, [state.currentNodeId]: result.errors[0]?.message ?? 'Campo invalido' },
          }
        }
      }
    ...
    case 'PREVIOUS': {
      ...
      return {
        ...state,
        currentNodeId: newHistory[newHistory.length - 1] ?? null,
        history: newHistory,
        direction: 'backward',
        isComplete: false,
      }
